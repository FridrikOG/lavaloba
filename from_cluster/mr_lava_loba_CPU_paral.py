...
from numba import vectorize, int64, float32, float64, prange
...
def add_Ellipse(patch):
    ellip = Ellipse([patch[0], patch[1]], 2*patch[2], 2 *
                    patch[3], patch[4], facecolor='none', edgecolor='k')
    ax.add_patch(ellip)
...
# No python true, parallize true


@njit(parallel=True)
def parallize_plot(to_plot_centers):
    for i in range(0, len(to_plot_centers)):
        plt.plot(to_plot_centers[i][0], to_plot_centers[i][1], 'o')


@jit(nopython=True, parallel=True)
def parallize_ellipse(to_ellipse_patch, ax, color):
    for i in prange(0, len(to_ellipse_patch)):
        # plt.plot(to_plot_centers[i][0],to_plot_centers[i][1],'o')
        patch = to_ellipse_patch[i]
        ax.add_patch(Ellipse([patch[0], patch[1]], 2*patch[2],
                             2*patch[3], patch[4], facecolor='none', edgecolor=color))
...
if __name__ == '__main__':
    ...
    if (volume_flag == 1):
        ...
        if (fixed_dimension_flag == 1):
            print("total_volume", total_volume, "n_flows", n_flows, "lobe_area",
                  lobe_area, "min_n_lobes", min_n_lobes, "max_n_lobes", max_n_lobes)
    ...
    if crop_flag:
        ...
        iW = np.maximum(
            0, (np.floor((np.min(x_vent)-west_to_vent-lx)/cell)).astype(int))
        iE = np.minimum(
            cols, (np.ceil((np.max(x_vent)+east_to_vent-lx)/cell)).astype(int))
        jS = np.maximum(
            0, (np.floor((np.min(y_vent)-south_to_vent-ly)/cell)).astype(int))
        jN = np.minimum(
            rows, (np.ceil((np.max(y_vent)+north_to_vent-ly)/cell)).astype(int))
        ...
        np.savetxt(output_DEM, np.flipud(arr),
    ...
    for flow in range(0, n_flows):
        ...
        if (a_beta == 0) and (b_beta == 0):
            ...
            n_lobes = int(np.ceil(np.random.uniform(
        ...
        if (n_flows > 1 and not ('SLURM_JOB_NAME' in os.environ.keys())):
            ...
            sys.stdout.write('\r')
            sys.stdout.write(
                "[%-20s] %d%% %s" % ('='*(last_percentage_5), last_percentage, est_rem_time))
            sys.stdout.flush()
        ...
        for i in range(0, n_init):
            ...
            if (n_vents == 1):
                ...
                if (vent_flag == 0):
                    ...
                    den = cum_fiss_length[idx_vent] - \
                        cum_fiss_length[idx_vent-1]
            ...
            if (saveshape_flag):
                ...
                w.record(
                    str(i+1), str(dist_int[i]), str(descendents[i]), str(0), str(flow+1))
        ...
        for i in range(n_init, n_lobes):
            ...
            '''HELLO YES HELP'''
            [xe, ye] = ellipse(x[idx], y[idx], x1[idx],
                               x2[idx], angle[idx], X_circle, Y_circle)
            ...
            if (plot_lobes_flag == 1):
                to_ellipse_patch2.append(
                    [x_new, y_new, new_x1, new_x2, new_angle])
            ...
            if (saveshape_flag):
                ...
                # compute the lobe
                [xe, ye] = ellipse(x_new, y_new, new_x1,
                                   new_x2, new_angle, X_circle, Y_circle)
            ...
            for i in range(0, last_lobe):
                ...
                if (i > 0):
                    ...
                    j_bottom_int = np.maximum(
                        j_bottom, jbottom_array[parent[i]])
                    ...
                    if (Zlocal_parent.shape[0] == 0 or Zlocal_parent.shape[1] == 0):
                        ...
                        print(
                            'j', jbottom_array[parent[i]], jtop_array[parent[i]])
                        print('i', ileft_array[parent[i]],
                              iright_array[parent[i]])
    ...
    for i in range(0, len(to_plot_centers)):
        plt.plot(to_plot_centers[i][0], to_plot_centers[i][1], 'o')
    # parallize_plot(to_plot_centers)
    ...
    paralStart = time.process_time()
    parallize_ellipse(to_ellipse_patch, ax, 'k')
    ...
    parallize_ellipse(to_ellipse_patch2, ax, 'r')
    if (plot_lobes_flag == 1):
        p = PatchCollection(patch, match_original=True)
        ax.add_collection(p)
    print("Elapsed time for paral ", paralStart)
    print('')
    print('')
    print('Total number of lobes', n_lobes_tot,
          'Average number of lobes', int(1.0*n_lobes_tot/n_flows))
    print('')
                ...
                for i in range(1, 10*max_lobes):
                    ...
                    if (coverage_fraction < masking_threshold[i_thr]):
                        ...
                        if (flag_threshold == 1):
                            ...
                            print('')
                            print('Masking threshold',
                                  masking_threshold[i_thr])
                            print('Total volume', cell**2*total_Zflow,
                                  ' m3 Masked volume', cell**2 *
                                  np.sum(masked_Zflow),
                        ...
                        with open(output_thickness, 'a') as the_file:
                            ...
                            if (i_thr == 0):
                                the_file.write(
                                    'Average lobe thickness = '+str(avg_lobe_thickness)+' m\n')
                                the_file.write(
                                    'Total volume = '+str(cell**2*total_Zflow)+' m3\n')
                                the_file.write(
                                    'Total area = '+str(cell**2*np.sum(Zflow > 0))+' m2\n')
                                the_file.write(
                                    'Average thickness full = '+str(total_Zflow/np.sum(Zflow > 0))+' m\n')

                            the_file.write(
                                'Masking threshold = '+str(masking_threshold[i_thr])+'\n')
                            the_file.write(
                                'Masked volume = '+str(cell**2*np.sum(masked_Zflow))+' m3\n')
                            the_file.write(
                                'Masked area = '+str(cell**2*np.sum(masked_Zflow > 0))+' m2\n')
                            the_file.write(
                                'Average thickness mask = '+str(np.sum(masked_Zflow) / np.sum(masked_Zflow > 0))+' m\n')
                ...
                if (masking_threshold[i_thr] < 1):
                    ...
                    for i in np.unique(Zhazard):
                        ...
                        if (flag_threshold == 1):
                            ...
                            volume_fraction = np.sum(
                                masked_Zflow) / total_Zflow
