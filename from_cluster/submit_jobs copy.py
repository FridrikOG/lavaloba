...
def main():
    ...
    with open(args.parameters,mode='r',newline='',encoding='utf-8-sig') as csvfile:
        ...
        created_dirs = []
        for job in reader:
            n_dir = "/scratch/hpc_2022/fjolnir/MrLavaLoba/{0}_{1}_{2}_{3}_{4}_{5}/".format(*job)
            if dir not in created_dirs:
                create_dirs(n_dir)
                created_dirs.append(n_dir)
            print(args.jobscript)
            job.append("/scratch/hpc_2022/fjolnir/MrLavaLoba/{0}_{1}_{2}_{3}_{4}_{5}/results".format(*job))
            submit_command = (
                "sbatch " +
                "-o /scratch/hpc_2022/fjolnir/MrLavaLoba/{0}_{1}_{2}_{3}_{4}_{5}/job_logs/out.txt ".format(*job) +
