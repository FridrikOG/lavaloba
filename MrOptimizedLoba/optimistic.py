...
@jit(nopython=False)
def slope_calc(Fy_test, Fx_test, pi ):
...
def get_rand_new_angle(slope, pi):
...
def main_program(run_name, source, vent_flag, x_vent, y_vent, hazard_flag, masking_threshold, n_flows, min_n_lobes, max_n_lobes, volume_flag, total_volume, fixed_dimension_flag, lobe_area, thickness_ratio, topo_mod_flag, n_flows_counter, n_lobes_counter, thickening_parameter, lobe_exponent, max_slope_prob, inertial_exponent, restart_files, saveshape_flag, saveraster_flag, flag_threshold, plot_lobes_flag, plot_flow_flag, a_beta, b_beta, force_max_length, max_length,n_check_loop,start_from_dist_flag, dist_fact, npoints, aspect_ratio_coeff, max_aspect_ratio, shape_nam):
    ...
    for flow in range(0, n_flows):
        ...
        for i in range(0, n_init):
            ...
            (max_slope_angle,slope ) = slope_calc(Fy_test, Fx_test, pi)
            ...
            if (max_slope_prob < 1):
                ...
                rand_angle_new = get_rand_new_angle(slope, pi)
